...
class VPNClientUI(QWidget):
    ...
    def read_stdout(self):
        """Handles standard output from the VPN process."""
        if self.vpn_process:
            output = self.vpn_process.readAllStandardOutput().data().decode()
            self.log_message(f"[VPN] {output.strip()}")

    def read_stderr(self):
        """Handles standard error from the VPN process."""
        if self.vpn_process:
            error_output = self.vpn_process.readAllStandardError().data().decode()
            self.log_message(f"[Error] {error_output.strip()}", "error")
